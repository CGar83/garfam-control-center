    ...
    <section className="hero-card fade-up p-5 sm:p-6" aria-label="Tonight's dinner">
      <div className="relative z-10 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex min-w-0 items-center gap-4">
          <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-3xl bg-accent text-3xl backdrop-blur-sm" aria-hidden>
            {meal ? recipe?.emoji ?? "🍴" : "🌙"}
          </span>
          <div className="min-w-0">
            <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Tonight</p>
        ...
        <div className="flex flex-wrap gap-2 sm:justify-end">
          {meal ? (
            <>
              {recipe ? (
                <Button variant="secondary" className="text-foreground" onClick={() => onOpenRecipe(recipe)}>
                  <BookOpen className="h-4 w-4" />
                  Open recipe
                </Button>
              ) : null}
              {meal.ingredients ? (
                <Button variant="outline" className="text-foreground" onClick={onAddIngredients} disabled={busy}>
            ...
            <>
              <Button variant="secondary" className="text-foreground" onClick={onPlan}>
                <Plus className="h-4 w-4" />
                Plan dinner
              </Button>
              {hasRecipes ? (
                <Button variant="outline" className="text-foreground" onClick={onPickForMe} disabled={busy}>
